...
export default function DashboardPage() {
  ...
  const connectYouTube = async () => {
    if (!ytApiKey.trim() || !ytHandle.trim()) { setYtError('Remplis les deux champs.'); return }
    setYtLoading(true); setYtError('')
    try {
      const handle = ytHandle.trim().startsWith('@') ? ytHandle.trim() : `@${ytHandle.trim()}`
      const byHandle = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&forHandle=${encodeURIComponent(handle)}&key=${ytApiKey.trim()}`)
      let json = await byHandle.json()
      if (!json.items?.length) {
        const byId = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&id=${ytHandle.trim()}&key=${ytApiKey.trim()}`)
        json = await byId.json()
      }
      if (json.error) throw new Error(json.error.message)
      if (!json.items?.length) throw new Error('Chaîne introuvable. Vérifie le handle ou l\'ID.')
      const ch = json.items[0]
      const data: YTData = { channelId: ch.id, title: ch.snippet.title, subscriberCount: ch.statistics.subscriberCount ?? '0', viewCount: ch.statistics.viewCount ?? '0', videoCount: ch.statistics.videoCount ?? '0', lastFetched: new Date().toISOString() }
      ...
      setYtData(data); setShowYTSetup(false)
  ...
  const disconnectYT = () => {
    ...
    setYtData(null); toggle('youtube'); setShowYTSetup(false)
  }
      ...
      <main style={{ marginLeft: '240px', padding: '40px 48px', minHeight: '100vh' }}>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '560px' }}>
            {/* YouTube */}
            <div>
              <div className="card-standard" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', opacity: connected.includes('youtube') ? 1 : 0.55 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '12px', color: '#94a3b8' }}>
                      {ytData ? `${ytData.title} · ${fmtNum(ytData.subscriberCount)} abonnés` : connected.includes('youtube') ? `${creator.pseudo} · données démo` : 'Déconnecté'}
                    </p>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  {ytData ? (
                    <span style={{ background: 'rgba(134,239,172,0.2)', color: '#15803d', border: '1px solid rgba(134,239,172,0.4)', borderRadius: '9999px', padding: '3px 12px', fontSize: '12px', fontWeight: 500 }}>Connecté ✓</span>
                  ) : connected.includes('youtube') ? (
                    <span style={{ background: '#fef9c3', color: '#854d0e', border: '1px solid #fde047', borderRadius: '9999px', padding: '3px 12px', fontSize: '12px', fontWeight: 500 }}>Données démo</span>
                  ) : (
                    <span style={{ background: '#f1f5f9', color: '#94a3b8', borderRadius: '9999px', padding: '3px 12px', fontSize: '12px', fontWeight: 500 }}>Déconnecté</span>
                  )}
                  {ytData ? (
                    <>
                      <button onClick={() => setShowYTSetup(v => !v)} title="Synchroniser" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', display: 'flex', padding: '2px' }}><RefreshCw size={14} /></button>
                      <button onClick={disconnectYT} style={{ fontSize: '12px', color: '#94a3b8', background: 'none', border: 'none', cursor: 'pointer' }}>Déconnecter</button>
                    </>
                  ) : (
                    <button onClick={() => setShowYTSetup(v => !v)} style={{ fontSize: '12px', color: '#16a34a', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}>
              ...
              {showYTSetup && (
                <div style={{ marginTop: '8px', padding: '20px 24px', background: 'white', border: '1.5px solid rgba(22,163,74,0.2)', borderRadius: '12px', position: 'relative' }}>
                  <button onClick={() => { setShowYTSetup(false); setYtError('') }} style={{ position: 'absolute', top: '12px', right: '12px', background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8' }}><X size={16} /></button>
                  <p style={{ fontSize: '13px', fontWeight: 600, color: '#0f172a', marginBottom: '4px' }}>Connecter ta chaîne YouTube</p>
                  <p style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '16px', lineHeight: 1.5 }}>
                    Crée une clé API gratuite sur <span style={{ color: '#16a34a', fontWeight: 500 }}>console.cloud.google.com</span> → Active &quot;YouTube Data API v3&quot; → Crée une clé.
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div>
                      <label style={labelStyle}>Clé API Google</label>
                      <input type="password" value={ytApiKey} onChange={e => setYtApiKey(e.target.value)} placeholder="AIzaSy..." style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>Handle ou ID de ta chaîne</label>
                      <input value={ytHandle} onChange={e => setYtHandle(e.target.value)} onKeyDown={e => e.key === 'Enter' && connectYouTube()} placeholder="@monpseudo ou UCxxxxx..." style={inputStyle} onFocus={focusStyle} onBlur={blurStyle} />
                    </div>
                    {ytError && <p style={{ fontSize: '12px', color: '#ef4444', background: '#fef2f2', padding: '8px 12px', borderRadius: '8px' }}>{ytError}</p>}
                    <button onClick={connectYouTube} disabled={ytLoading} style={{ alignSelf: 'flex-start', background: ytLoading ? '#86efac' : '#16a34a', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 20px', fontSize: '13px', fontWeight: 600, cursor: ytLoading ? 'wait' : 'pointer' }}>
            ...
            {platforms.filter(p => p.id !== 'youtube').map(p => {
              ...
              return (
                <div key={p.id} className="card-standard" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', opacity: isConnected ? 1 : 0.55 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <PlatformIcon id={p.id} color={isConnected ? p.color : '#94a3b8'} />
                    <div>
                      <p style={{ fontSize: '14px', fontWeight: 600, color: '#0f172a' }}>{p.name}</p>
                      <p style={{ fontSize: '12px', color: '#94a3b8' }}>{isConnected ? `${creator.pseudo} · ${p.mainStat.value} ${p.mainStat.label}` : 'Déconnecté'}</p>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    {isConnected ? (
                    ...
                    <button onClick={() => toggle(p.id)} style={{ fontSize: '12px', color: isConnected ? '#94a3b8' : '#16a34a', fontWeight: isConnected ? 400 : 600, background: 'none', border: 'none', cursor: 'pointer' }}>
